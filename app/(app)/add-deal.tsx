...
export default function AddDealScreen() {
    ...
    const availablePrograms = useMemo(() => {
        ...
        return selectedUniObj.programs.filter((p: any) => p.name?.toLowerCase().includes(debouncedProgSearch.toLowerCase()));
    ...
    const handleProgramSelect = (prog: any) => {
        ...
        if (currencyObj && currencyObj.code !== 'USD') {
            ...
            setConversionInfo(`Обучение: $${Math.round(tuitionUsd)} + Услуги: $${Math.round(serviceUsd)}`);
        }
    };
    ...
    const submitForm = async () => {
        ...
        if (dealType === 'university' && (!universityId || !programId)) return Alert.alert("Ошибка", "Выберите ВУЗ и программу");
        if (dealType === 'service' && !customServiceName.trim()) return Alert.alert("Ошибка", "Введите название услуги");
        ...
        try {
            ...
            if (String(selectedClient).startsWith('temp_')) throw new Error("offline");

            await apiClient.post('analytics/deals/', payload);
            Alert.alert('Успех', 'Сделка успешно создана', [{ text: 'OK', onPress: () => router.replace('/crm') }]);

        } catch (error: any) {
            if (error.response?.data) {
                let errorMsg = Object.entries(error.response.data).map(([key, msgs]) => `${key}: ${msgs}`).join('\n');
                Alert.alert('Ошибка сервера', errorMsg);
            } else {
                const offlineDeals = JSON.parse(await getToken('offline_deals') || '[]');
                offlineDeals.push({
                    ...payload, id: `temp_${Date.now()}`, client_name: clientName || "Офлайн клиент", payment_status: 'new', isOffline: true
                });
                await saveToken('offline_deals', JSON.stringify(offlineDeals));
                Alert.alert("Нет сети", "Сделка сохранена локально.");
                router.replace('/crm');
            }
        } finally {
                ...
                <View style={styles.card}>
                    ...
                    <View style={styles.rowInputs}>
                        <TouchableOpacity style={[styles.typeChip, dealType === 'university' && styles.typeChipActive]} onPress={() => setDealType('university')}>
                            <Ionicons name="school" size={18} color={dealType === 'university' ? "#fff" : Colors.light.textSecondary} style={{marginRight: 6}} />
                        ...
                        <TouchableOpacity style={[styles.typeChip, dealType === 'service' && styles.typeChipActive]} onPress={() => {setDealType('service'); setUniversityId(''); setProgramId(''); setConversionInfo('');}}>
                            <Ionicons name="airplane" size={18} color={dealType === 'service' ? "#fff" : Colors.light.textSecondary} style={{marginRight: 6}} />
                    ...
                    {dealType === 'university' && (
                        <>
                            ...
                            <View style={styles.searchBox}>
                                ...
                                <TextInput style={styles.searchInputInner} placeholder="Название ВУЗа..." placeholderTextColor={Colors.light.textSecondary} value={uniSearch} onChangeText={setUniSearch} />
                            </View>
                            <ScrollView style={styles.verticalList} nestedScrollEnabled={true} keyboardShouldPersistTaps="handled">
                                {filteredUnis.map(u => (
                                    <TouchableOpacity key={u.id} style={[styles.listItem, universityId === u.id && styles.listItemSelected]} onPress={() => { setUniversityId(u.id); setProgramId(''); setProgSearch(''); setConversionInfo(''); Keyboard.dismiss(); }}>
                                        ...
                                        {universityId === u.id && <Ionicons name="checkmark-circle" size={20} color={Colors.light.primary} />}
                                    </TouchableOpacity>
                                ))}
                            </ScrollView>
                            ...
                            {universityId !== '' && (
                                <>
                                    ...
                                    <View style={styles.searchBox}>
                                        <Ionicons name="search" size={18} color={Colors.light.textSecondary} style={{marginRight: 8}} />
                                        <TextInput style={styles.searchInputInner} placeholder="Поиск программы..." placeholderTextColor={Colors.light.textSecondary} value={progSearch} onChangeText={setProgSearch} />
                                    </View>
                                    <ScrollView style={styles.verticalList} nestedScrollEnabled={true} keyboardShouldPersistTaps="handled">
                                        {availablePrograms.map((p: any) => (
                                            <TouchableOpacity key={p.id} style={[styles.listItem, programId === p.id && styles.listItemSelected]} onPress={() => handleProgramSelect(p)}>
                                                <View style={{flex: 1}}>
                                                    ...
                                                    <Text style={styles.listItemSubText}>{degreeMap[p.degree] || p.degree} | {p.duration}</Text>
                                                </View>
                                                {programId === p.id && <Ionicons name="checkmark-circle" size={20} color={Colors.light.primary} />}
                                            </TouchableOpacity>
                                        ))}
                                    </ScrollView>
                                </>
                    ...
                    {dealType === 'service' && (
                        <>
                            ...
                            <TextInput style={styles.input} placeholder="Например: Оформление визы" placeholderTextColor={Colors.light.textSecondary} value={customServiceName} onChangeText={setCustomServiceName} />

                            <Text style={styles.label}>Описание действий</Text>
                            <TextInput style={[styles.input, { height: 80, textAlignVertical: 'top' }]} multiline placeholder="Собрать документы..." placeholderTextColor={Colors.light.textSecondary} value={customServiceDesc} onChangeText={setCustomServiceDesc} />
                        </>
                    )}
                    ...
                    {conversionInfo !== '' && dealType === 'university' && (
                        <View style={styles.infoBox}>
                            <Ionicons name="information-circle-outline" size={18} color={Colors.light.primary} style={{marginRight: 8}} />
                    ...
                    <View style={styles.rowInputs}>
                        <View style={{flex: 1, marginRight: 10}}>
                            ...
                            <TextInput style={styles.searchBox} keyboardType="numeric" placeholder="0" placeholderTextColor={Colors.light.textSecondary} value={priceClient} onChangeText={setPriceClient} />
                        </View>
                        <View style={{flex: 1}}>
                            <Text style={styles.label}>Прибыль ($)</Text>
                            <TextInput style={styles.searchBox} keyboardType="numeric" placeholder="0" placeholderTextColor={Colors.light.textSecondary} value={expectedRevenue} onChangeText={setExpectedRevenue} />
                        </View>
                    </View>
...
const styles = StyleSheet.create({
    ...
    backBtn: { padding: 10, backgroundColor: '#fff', borderRadius: Layout.radius.small, ...Layout.shadows.light },
    title: { fontSize: 20, fontWeight: '800', color: Colors.light.text },
    container: { paddingHorizontal: 20, paddingBottom: 100 },
    
    card: { padding: 24, borderRadius: Layout.radius.large, backgroundColor: '#FFFFFF', ...Layout.shadows.light, marginBottom: 20 },
    sectionSubTitle: { color: Colors.light.primary, fontSize: 13, fontWeight: '800', marginBottom: 15, marginTop: 10, textTransform: 'uppercase', letterSpacing: 1 },
    label: { fontSize: 11, fontWeight: '800', color: Colors.light.textSecondary, marginBottom: 8, marginLeft: 6, textTransform: 'uppercase' },
    
    input: { backgroundColor: '#F2F2F7', borderRadius: Layout.radius.medium, padding: 16, fontSize: 15, marginBottom: 20, color: Colors.light.text, fontWeight: '500' },
    inputReadOnly: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F2F2F7', borderRadius: Layout.radius.medium, padding: 16, marginBottom: 20 },
    
    searchBox: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#F2F2F7', borderRadius: Layout.radius.medium, paddingHorizontal: 15, height: 50, marginBottom: 10, fontSize: 16, fontWeight: '600' },
    searchInputInner: { flex: 1, color: Colors.light.text, fontSize: 16, fontWeight: '500', outlineStyle: 'none' },
    
    verticalList: { maxHeight: 200, backgroundColor: '#F2F2F7', borderRadius: Layout.radius.medium, padding: 5, marginBottom: 20 },
    listItem: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', padding: 14, borderRadius: Layout.radius.small, marginBottom: 2 },
    ...
    listItemSubText: { color: Colors.light.textSecondary, fontSize: 11, marginTop: 4, fontWeight: '500' },

    rowInputs: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 20 },
    typeChip: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: '#F2F2F7', paddingVertical: 12, borderRadius: Layout.radius.medium, marginRight: 10 },
    ...
    modalChip: { backgroundColor: '#F2F2F7', paddingHorizontal: 16, paddingVertical: 12, borderRadius: Layout.radius.medium, marginRight: 10 },
    modalChipActive: { backgroundColor: Colors.light.primary },
    modalChipText: { fontSize: 13, fontWeight: '700', color: Colors.light.text },
    
    infoBox: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(0, 122, 255, 0.1)', padding: 12, borderRadius: Layout.radius.small, marginBottom: 15 },
    infoBoxText: { flex: 1, color: Colors.light.primary, fontSize: 12, fontWeight: '600' },

    submitBtn: { backgroundColor: Colors.light.primary, padding: 18, borderRadius: Layout.radius.large, alignItems: 'center', marginTop: 10, ...Layout.shadows.medium },
