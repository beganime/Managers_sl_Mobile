import { useRouter } from 'expo-router';
import React, { useCallback, useState } from 'react';
import { ActivityIndicator, FlatList, StyleSheet, View } from 'react-native';

import { listApplications } from '../../api/crm';
import { ApiListItem } from '../../types';
import { Button } from '../../components/ui/Button';
import { EmptyState } from '../../components/ui/EmptyState';
import { ErrorState } from '../../components/ui/ErrorState';
import { Header } from '../../components/layout/Header';
import { ScreenContainer } from '../../components/layout/ScreenContainer';
import { theme } from '../../theme/theme';
import { useDebouncedValue } from '../../hooks/useDebouncedValue';
import { usePagedResource } from '../../hooks/usePagedResource';
import { CrmListCard, SearchInput } from './components';

export function ApplicationsScreen() {
  const router = useRouter();
  const [search, setSearch] = useState('');
  const debouncedSearch = useDebouncedValue(search);

  const loader = useCallback(
    ({ limit, offset }: { limit: number; offset: number }) =>
      listApplications({ limit, offset, search: debouncedSearch.trim() || undefined }),
    [debouncedSearch]
  );

  const { items, loading, refreshing, loadingMore, error, refresh, loadMore } =
    usePagedResource<ApiListItem>(loader);

  return (
    <ScreenContainer scroll={false}>
      <Header title="Заявки на поступление" subtitle="CRM applications из /api/v1/crm/applications/." showBack />

      <View style={styles.tools}>
        <SearchInput value={search} onChangeText={setSearch} placeholder="Поиск по клиенту или вузу" />
        <Button title="Создать заявку" onPress={() => router.push('/(app)/crm/applications/create' as any)} />
      </View>

      {error && !items.length ? <ErrorState message={error} actionTitle="Повторить" onAction={refresh} /> : null}

      <FlatList
        data={items}
        keyExtractor={(item, index) => String(item.id || index)}
        contentContainerStyle={styles.list}
        refreshing={refreshing}
        onRefresh={refresh}
        onEndReached={loadMore}
        onEndReachedThreshold={0.5}
        ListEmptyComponent={
          loading ? null : <EmptyState title="Заявок пока нет" message="Создайте заявку для клиента." />
        }
        ListFooterComponent={loadingMore ? <ActivityIndicator color={theme.colors.accent} /> : null}
        renderItem={({ item }) => (
          <CrmListCard
            item={item}
            type="application"
            actionLabel="Открыть"
            onPress={() => router.push(`/(app)/crm/applications/${item.id}` as any)}
          />
        )}
      />
    </ScreenContainer>
  );
}

const styles = StyleSheet.create({
  tools: {
    gap: theme.spacing.md,
    paddingBottom: theme.spacing.md,
  },
  list: {
    gap: theme.spacing.md,
    paddingBottom: 128,
  },
});
